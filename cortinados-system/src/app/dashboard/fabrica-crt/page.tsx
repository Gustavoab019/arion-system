...
export default function FabricaCRTDashboard() {
  ...
  // Carregar itens do tipo cortina
  useEffect(() => {
    carregarItens();
  }, [filter]);
  ...
  const carregarItens = async () => {
    try {
      ...
      const response = await fetch(`/api/items/type/cortina?${statusFilter}`);
  ...
  const atualizarStatus = async (itemId: string, novoStatus: string) => {
    try {
      ...
      const response = await fetch(`/api/items/${itemId}/status`, {
        ...
        body: JSON.stringify({
          status: novoStatus,
          observacoes: `Status atualizado pela fábrica CRT - ${session?.user?.name}`
        }),
      });
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <Loading size="lg" text="Carregando itens CRT..." />
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8 space-y-8">
            ...
            <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-6 text-center sm:text-left">
              ...
              <div>
                <h2 className="text-2xl lg:text-4xl font-bold text-gray-900 mb-2">
                  Centro de Produção CRT
                </h2>
                <p className="text-gray-700 mb-4 text-lg lg:text-xl">
                  Gestão de Produção de Cortinas
                </p>
                <p className="text-gray-600 mb-6 text-base lg:text-lg">
                  Controle completo do fluxo de produção de cortinas
                </p>
              </div>
            </div>
        ...
        {itens.length === 0 ? (
          <div className="bg-white rounded-xl border-4 border-gray-200 p-8 shadow-lg">
            <div className="text-center py-12">
              ...
              <h3 className="text-2xl font-bold text-gray-900 mb-4">
                Nenhuma cortina encontrada
              </h3>
              <p className="text-gray-600 text-lg">
                {filter === 'medido' 
                  ? 'Não há cortinas aguardando produção no momento.'
                  : `Não há cortinas com status "${filter}".`
                }
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {itens.map((item) => {
              ...
              return (
                <div key={item._id} className="bg-white rounded-xl border-4 border-gray-200 hover:border-emerald-400 hover:shadow-xl transition-all duration-300 p-6 shadow-lg">
                  <div className="space-y-4">
                    ...
                    {item.medidas && (
                      <div className="bg-emerald-50 p-4 rounded-xl border-2 border-emerald-200">
                        <h4 className="font-bold text-emerald-900 mb-2 text-lg">📐 Medidas:</h4>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-white rounded-xl border-4 border-yellow-200 p-6 lg:p-8 shadow-lg hover:shadow-xl transition-all duration-200">
            ...
            <div className="text-sm lg:text-base text-gray-600">Costurando</div>
          </div>
          ...
          <div className="bg-white rounded-xl border-4 border-emerald-200 p-6 lg:p-8 shadow-lg hover:shadow-xl transition-all duration-200">
            <div className="flex items-center justify-between mb-4">
              <span className="text-gray-700 text-base lg:text-lg font-bold">Total CRT</span>
              <div className="w-4 h-4 bg-emerald-500 rounded-full"></div>
            </div>
            <div className="text-3xl lg:text-4xl font-bold text-gray-900 mb-2">
              {itens.length}
            </div>
            <div className="text-sm lg:text-base text-gray-600">Cortinas total</div>
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-4">
              <div className="flex items-center space-x-3">
                ...
                <span className="text-gray-900 font-bold text-lg">Fábrica CRT Operacional</span>
              </div>
              <div className="hidden sm:block text-gray-400">•</div>
              <span className="text-gray-600 text-lg font-semibold">
                Produção de Cortinas Ativa
              </span>
            </div>
